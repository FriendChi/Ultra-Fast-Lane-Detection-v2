...
def merge_config():
    ...
    'num_cell_row', 'num_cell_col', 'mean_loss_w','fc_norm','soft_loss','cls_loss_col_w', 'cls_ext_col_w', 'mean_loss_col_w', 'eval_mode', 'eval_during_training', 'split_channel', 'match_method', 'selected_lane', 'cumsum', 'masked','train_txt_root']
    for item in items:
...
def get_train_loader(cfg):
    if cfg.dataset == 'CULane':
        ...
        train_loader = TrainCollect(cfg.batch_size, 4, cfg.data_root, cfg.train_txt_root, get_rank(), get_world_size(), 
                                cfg.row_anchor, cfg.col_anchor, cfg.train_width, cfg.train_height, cfg.num_cell_row, cfg.num_cell_col, cfg.dataset, cfg.crop_ratio)
    elif cfg.dataset == 'CurveLanes':
        train_loader = TrainCollect(cfg.batch_size, 4, cfg.data_root, os.path.join(cfg.train_txt_root, 'train', 'train_gt.txt'), get_rank(), get_world_size(), 
                                cfg.row_anchor, cfg.col_anchor, cfg.train_width, cfg.train_height, cfg.num_cell_row, cfg.num_cell_col, cfg.dataset, cfg.crop_ratio)
    else:
