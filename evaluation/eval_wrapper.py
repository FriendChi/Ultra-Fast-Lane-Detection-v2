...
def eval_lane(net, cfg, ep = None, logger = None):
    ...
    if cfg.dataset == 'CurveLanes':
        ...
        if is_main_process():
            ...
            res = LaneEval.bench_one_submit(os.path.join(cfg.test_work_dir, exp_name + '.txt'), os.path.join('/kaggle/input/tusimple/TUSimple', 'test_label.json'))            res = json.loads(res)
            for r in res:
