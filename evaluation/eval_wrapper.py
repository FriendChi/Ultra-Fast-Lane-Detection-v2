...
def eval_lane(net, cfg, ep = None, logger = None):
    ...
    if cfg.dataset == 'CurveLanes':
        ...
        if is_main_process():
            ...
            res = LaneEval.bench_one_submit(os.path.join(cfg.test_work_dir,exp_name + '.txt'),os.path.join(os.path.dirname(cfg.data_root),'test_label.json'))
            res = json.loads(res)
            for r in res:
...
def call_curvelane_eval(data_dir, exp_name,output_path):
    ...
    if platform.system() == 'Windows':
        eval_cmd = eval_cmd.replace('/', os.sep)
    data_dir = '/kaggle/input/curve-val/images/'
