...
def get_test_loader(batch_size, data_root,dataset, distributed, crop_ratio, train_width, train_height):
    ...
    if dataset == 'CULane':
        ...
        test_dataset = LaneTestDataset(data_root,os.path.join(data_root, 'list/list/test.txt'),img_transform = img_transforms, crop_size=train_height)
    elif dataset == 'Tusimple':
        img_transforms = transforms.Compose([
            transforms.Resize((int(train_height / crop_ratio), train_width)),
            transforms.ToTensor(),
        ...
        test_dataset = LaneTestDataset(data_root.replace('train_set','test_set'),os.path.join('/kaggle/working/', 'test.txt'), img_transform = img_transforms, crop_size=train_height)
        ...
        test_dataset = LaneTestDataset(data_root,'/kaggle/working/valid_for_culane_style.txt',img_transform = img_transforms, crop_size=train_height)
    else:
        raise NotImplementedError
    if distributed:
    ...
    loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, sampler = sampler, num_workers=1)
    return loader
